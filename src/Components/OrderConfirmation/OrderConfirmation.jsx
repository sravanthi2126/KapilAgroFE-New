...
const OrderConfirmation = () => {
  ...
  useEffect(() => {
    ...
    const fetchInvoiceDetails = async (invoiceId) => {
      try {
        const token = localStorage.getItem('token');
        const response = await apiClient.get(`/user/invoice/${invoiceId}`, {
          headers: { 'Authorization': `Bearer ${token}` },
        });
        ...
        if (response.status === 200 && response.data.status === 'success') {
          ...
          console.warn('Invoice fetch failed or not ready:', response.data?.message || 'No invoice data');
          showError('Invoice not available yet. Please check later or contact support.', { autoClose: false });
        }
      } catch (error) {
        console.error('Error auto-fetching invoice:', error);
        showError('Failed to load invoice details. Please try again later.', { autoClose: false });
      }
    };

    const orderId = location.state?.orderId || new URLSearchParams(location.search).get('orderId');
    const invoiceId = location.state?.invoiceId; // Use invoiceId from payment flow
    if (orderId) {
      console.log('Fetching order details for orderId:', orderId);
      fetchOrderDetails(orderId);
      if (invoiceId) {
        console.log('Fetching invoice details for invoiceId:', invoiceId);
        fetchInvoiceDetails(invoiceId);
      } else {
        console.warn('No invoiceId provided, invoice may not be available yet.');
        showError('Invoice ID not available yet. Please check later.', { autoClose: false });
      }
    } else {
      setLoading(false);
      showError('Order ID not found. Please check your order history.', { autoClose: false });
          ...
          <div className="card-content">
            ...
            <div className="detail-row">
              ...
              <span className="detail-value detail-value-small">
                {invoiceDetails?.invoiceId || 'Pending'}
              </span>
            </div>

            <div className="detail-row">
              <span className="detail-label">Order Date</span>
              <span className="detail-value">{formatDate(orderDetails.placedAt || new Date())}</span>
            </div>
