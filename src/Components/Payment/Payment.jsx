...
const Payment = ({ cart, setCart, setIsLoginOpen }) => {
    ...
    useEffect(() => {
        ...
        if (initialOrderDetails) {
            ...
            const basicOrderDetails = {
                ...
                razorpayOrderId: null, // Will be set after /initiate
            };
    ...
    const recalculateOrderDetails = useCallback(async () => {
        ...
        try {
            ...
            if (!token) {
                ...
                // Trigger client-side login flow if token is missing here as a fallback
                navigate('/', { replace: true });
                toast.error('Session expired. Please log in again.', { onClick: () => setIsLoginOpen(true) });
                return;
            }
            ...
            if (response.status === 201 && response.data.status === 'success') {
                ...
                setCurrentOrderDetails(prev => ({
                    ...
                    razorpayOrderId: newDetails.razorpayOrderId, // CRITICAL: Save RazorpayOrderId
    ...
    }, [cart, shippingAddress, billingAddress, navigate, setIsLoginOpen]); // Added navigate/setIsLoginOpen dependencies
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                const options = {
                    ...
                    order_id: orderData.razorpayOrderId, // Use the ID generated by /initiate
    ...
    const handlePlaceOrder = async () => {
        ...
        try {
            // CRITICAL: Re-trigger recalculate/initiate one last time to ensure the Razorpay Order ID is up to date 
            // and the latest totals are used.
            if (!currentOrderDetails.razorpayOrderId) {
                await recalculateOrderDetails();
            }
